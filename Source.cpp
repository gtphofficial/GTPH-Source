...
#include "gtph_ItemDB.h"
#include "gtph_Base.h"
#include "gtph_Players.h"
#include "gtph_Packets.h"
#include "gtph_Guilds.h"
#include "gtph_skStr.h"
#include "gtph_Worlds.h"
...
void loop_worlds() {
	if (server_port != main_port && server_port != 16154) return;
	if (f_saving_ == false) {
		...
		if (last_time - ms_time <= 0) {
			...
			if (Crypto_Update.crypto_time - time(nullptr) <= 0) {
				...
				string info = "";
				std::ifstream ifs("database/crypto_prices.json");
		...
		if (last_autofarm - ms_time <= 0) {
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (pInfo(currentPeer)->cheater_settings & GTPH::SETTINGS_0 && pInfo(currentPeer)->disable_cheater == 0) {
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (pInfo(currentPeer)->fishing_used != 0) {
					if (pInfo(currentPeer)->last_fish_catch + pInfo(currentPeer)->fish_seconds < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() && rand() % 100 < (pInfo(currentPeer)->hand == 6258 ? 20 : pInfo(currentPeer)->hand == 3010 ? 15 : 10)) {
						...
						delete[] raw;
						if (pInfo(currentPeer)->cheater_settings & GTPH::SETTINGS_16 && pInfo(currentPeer)->disable_cheater == 0) {
...
int main(int argc, char* argv[]) {
	...
	/*
	{
		ifstream ifs("database/volcanic_capes.json");
		if (ifs.is_open()) {
			json j;
			ifs >> j;
	...
	for (const auto& entry : fs::directory_iterator("worlds")) {
		if (!fs::is_directory(entry.path())) {
			...
			string path2_ = "database/worlds/" + world_text + "_.json";
	...
	if (init_enet(server_port) == -1) cout << "enet error" << endl;
	else {
		if (server_port == 16154) domain = "https://www.gtphprivateserver.com/", main_port = 16154;
		if (items_dat() == -1) cout << "items.dat not loaded" << endl;
		else cout << setGems(items.size()) << "items loaded, with HASH: " << item_hash << endl;
		//cout << "[GTPH]" << "|PORT:" << server_port << "|MAX ITEMS : " << setGems(items.size()) << "| " << endl;
	}
	Server_Security.up_time_ = time(nullptr);
	system("cls");
	system("color 4");
	cout << "[+] GTPH Private Server Family" << endl;
	cout << "[+] Team GTPH Private Server" << endl;
	cout << "[+] Pinas Private Server Since 2022" << endl;
	cout << "[+] Server is now up" << endl;
	cout << "[+] (C) Copyright Alrights Reserved GTPH 2022-2023-2024" << endl;
	cout << "[+] Written Source Using C++ Update & Coded By: GTPH Development Team" << endl;
	cout << "[+] Contact Number: +63-9641601680" << endl;
	cout << "[+] GTPH Database Has Been Loaded" << endl;
	cout << "[+] Listening on Active Server Port: " << server_port << endl;
	cout << "[+] Total Count On: " << setGems(items.size()) << " Max Items" << endl;
	cout << "[+] OnSuperMain Has Been Loaded" << osm_link << endl;
	cout << "[+] CDN Has Been Loaded " << osm_path << endl;
	cout << "[+] Server Proto Has Been Loaded" << proto << endl;
	for (int i = 0; i < updates.size(); i++) {
		update_List += "\nadd_spacer|small|\nadd_textbox| `7[```9/```0]`` `$" + updates[i] + "``|left|";
		if (i < 20) update_news_list += "\nadd_label_with_icon|small|" + updates[i] + "|left|24|";
		
	}
	ifstream GTPH("database/config.json");
	if (!GTPH.is_open()) {
		cout << "www.growtopia1.com: ";
		cin >> osm_link;
		cout << "cache/: ";
		cin >> osm_path;
		ofstream GTPHteam("database/config.json");
		json j;
	...
	while (true) {
		while (enet_host_service(server, &event, 3000) > 0 && (f_saving_ == false and auto_save == false)) {
			...
			case ENET_EVENT_TYPE_CONNECT:
			{
				...
				variants::OnConsoleMessage(peer, "`6Connected To `# " + server_name + " ``");
				variants::OnConsoleMessage(peer, "`eFacebook Group: https://www.facebook.com/groups/gtpsphilippines");
				variants::OnConsoleMessage(peer, "`eFacebook Group: https://www.facebook.com/groups/GTPHOfficial");
				variants::OnConsoleMessage(peer, "`2We are back on GTPH (Growtopia Philippines)");
				variants::OnConsoleMessage(peer, "`8Server is `5Coded By `6@ZLS");
				variants::OnConsoleMessage(peer, "`9Pinas Since 2022 & GTPH Since 2021-2024");
				variants::OnConsoleMessage(peer, "`4Official Website:https://www.gtphprivateserver.com");
				...
				case 2:
				{
					...
					*/
					...
					if (server_port == 2) {
						...
						ofstream m;
						m.open("database/logs/"+ pInfo(peer)->tankIDName +".txt", std::ios_base::app), m << log, m.close();
					}*/
					loop_worlds();
					...
					if (pInfo(peer)->trading_with == -1) {
						...
						break;
							...
							else if (role == "dev" && domain == "https://www.gtphprivateserver.com/") price = role_price.dev_price;
							else if (role == "ghost" && domain == "https://www.gtphprivateserver.com/") price = role_price.ghost_price;
							gamepacket_t p;
							p.Insert("OnConsoleMessage");
							if (price != 0 && pInfo(peer)->gtwl >= price) {
								...
								else if (role == "dev" && domain == "https://www.gtphprivateserver.com/") {
									pInfo(peer)->dev = 1;
									pInfo(peer)->mod = 1;
									p.Insert("`o>> You purchased developer!");
								}
								else if (role == "ghost" && domain == "https://www.gtphprivateserver.com/") {
						...
						break;
							...
							if (p != worlds.end()) {
								...
								if (pInfo(peer)->cheater_settings & GTPH::SETTINGS_5 && pInfo(peer)->disable_cheater == 0) {
									string name_ = pInfo(peer)->world;
									vector<World>::iterator p = find_if(worlds.begin(), worlds.end(), [name_](const World& a) { return a.name == name_; });
									if (p != worlds.end()) {
										World* world_ = &worlds[p - worlds.begin()];
							...
							if (pInfo(peer)->cheater_settings & GTPH::SETTINGS_15 && pInfo(peer)->disable_cheater == 0) {
								int count = c_;
								c_ = c_ * -1;
								if (modify_inventory(peer, id_, c_) == 0) {
									...
									gamepacket_t p;
									p.Insert("OnConsoleMessage");
									if (pInfo(peer)->supp != 0 && domain != "https://www.gtphprivateserver.com/") {
							...
							if (pInfo(peer)->netID == netID) {
								...
								for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
									...
									if (pInfo(currentPeer)->netID == netID and pInfo(currentPeer)->world == pInfo(peer)->world) {
										...
										if (pInfo(peer)->cheater_settings & GTPH::SETTINGS_14 && pInfo(peer)->disable_cheater == 0) {
							...
							if (((force_double_jump != 0 && block_double_jump != 0) && force_double_jump == block_double_jump) || ((force_high_jump != 0 && block_high_jump != 0) && force_high_jump == block_high_jump) || ((force_heat_resist != 0 && block_heat_resist != 0) && force_heat_resist == block_heat_resist) || ((force_strong_punch != 0 && block_strong_punch != 0) && force_strong_punch == block_strong_punch) || ((force_long_punch != 0 && block_long_punch != 0) && force_long_punch == block_long_punch) || ((force_speedy != 0 && block_speedy != 0) && force_speedy == block_speedy) || ((force_long_build != 0 && block_long_build != 0) && force_long_build == block_long_build)) {
								...
								if (paa != worlds.end()) {
									...
									if (block_->fg == 2072 && block_access(peer, world_, &world_->blocks[pInfo(peer)->lastwrenchx + (pInfo(peer)->lastwrenchy * 100)])) {
										if (force_double_jump) world_->xenonite |= GTPH::XENONITE_FORCE_DOUBLE_JUMP;
										else world_->xenonite &= ~GTPH::XENONITE_FORCE_DOUBLE_JUMP;

										if (block_double_jump) world_->xenonite |= GTPH::XENONITE_BLOCK_DOUBLE_JUMP;
										else world_->xenonite &= ~GTPH::XENONITE_BLOCK_DOUBLE_JUMP;

										if (force_high_jump) world_->xenonite |= GTPH::XENONITE_FORCE_HIGH_JUMP;
										else world_->xenonite &= ~GTPH::XENONITE_FORCE_HIGH_JUMP;

										if (block_high_jump) world_->xenonite |= GTPH::XENONITE_BLOCK_HIGH_JUMP;
										else world_->xenonite &= ~GTPH::XENONITE_BLOCK_HIGH_JUMP;

										if (force_heat_resist) world_->xenonite |= GTPH::XENONITE_FORCE_HEAT_RESIST;
										else world_->xenonite &= ~GTPH::XENONITE_FORCE_HEAT_RESIST;

										if (block_heat_resist) world_->xenonite |= GTPH::XENONITE_BLOCK_HEAT_RESIST;
										else world_->xenonite &= ~GTPH::XENONITE_BLOCK_HEAT_RESIST;

										if (force_strong_punch) world_->xenonite |= GTPH::XENONITE_FORCE_STRONG_PUNCH;
										else world_->xenonite &= ~GTPH::XENONITE_FORCE_STRONG_PUNCH;

										if (block_strong_punch) world_->xenonite |= GTPH::XENONITE_BLOCK_STRONG_PUNCH;
										else world_->xenonite &= ~GTPH::XENONITE_BLOCK_STRONG_PUNCH;

										if (force_long_punch) world_->xenonite |= GTPH::XENONITE_FORCE_LONG_PUNCH;
										else world_->xenonite &= ~GTPH::XENONITE_FORCE_LONG_PUNCH;

										if (block_long_punch) world_->xenonite |= GTPH::XENONITE_BLOCK_LONG_PUNCH;
										else world_->xenonite &= ~GTPH::XENONITE_BLOCK_LONG_PUNCH;

										if (force_speedy) world_->xenonite |= GTPH::XENONITE_FORCE_SPEEDY;
										else world_->xenonite &= ~GTPH::XENONITE_FORCE_SPEEDY;

										if (block_speedy) world_->xenonite |= GTPH::XENONITE_BLOCK_SPEEDY;
										else world_->xenonite &= ~GTPH::XENONITE_BLOCK_SPEEDY;


										if (force_long_build) world_->xenonite |= GTPH::XENONITE_FORCE_LONG_BUILD;
										else world_->xenonite &= ~GTPH::XENONITE_FORCE_LONG_BUILD;

										if (block_long_build) world_->xenonite |= GTPH::XENONITE_BLOCK_LONG_BUILD;
										else world_->xenonite &= ~GTPH::XENONITE_BLOCK_LONG_BUILD;
						...
						break;
							...
							if (pInfo(peer)->logger) {
								...
								if (t_.size() > 3) {
									button = explode("\n", t_[3])[0].c_str();
									if (button == "Empty Logs" && to_lower(pInfo(peer)->tankIDName) == "ISeven") Server_Security.logs.clear();
								}
								if (t_.size() > 5) {
									search = explode("\n", t_[5])[0].c_str();
									if (search == "Next page") {
						...
						if (t_.size() == 29) {
							...
							if (time_dilation) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_TIME_DILATION_ON;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_TIME_DILATION_ON;
							...
							if (open_wings) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_OPEN_WINGS;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_OPEN_WINGS;

							bool closed_wings = atoi(explode("\n", t_[8])[0].c_str());
							if (closed_wings) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_CLOSE_WINGS;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_CLOSE_WINGS;

							bool stamp_particle = atoi(explode("\n", t_[9])[0].c_str());
							if (stamp_particle) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_STAMP_PARTICLE;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_STAMP_PARTICLE;

							bool trail0 = atoi(explode("\n", t_[10])[0].c_str());
							if (trail0) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_TRAIL_ON;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_TRAIL_ON;


				

							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_PORTAL_AURA;
							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_PORTAL_AURA;
							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_STARFIELD_AURA;
							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_STARFIELD_AURA;

							bool p_aura = atoi(explode("\n", t_[11])[0].c_str()), starfield_Aura = atoi(explode("\n", t_[12])[0].c_str()), electrical_aura = atoi(explode("\n", t_[13])[0].c_str());
							if (electrical_aura)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_ELECTRICAL_AURA;
							else if (p_aura)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_PORTAL_AURA;
							...
							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_MATERIAL_FEATHERS;
							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_MATERIAL_FEATHERS;
							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_1_MATERIAL_BLADES;
							pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_MATERIAL_BLADES;

							bool feathers = atoi(explode("\n", t_[14])[0].c_str()), blades = atoi(explode("\n", t_[15])[0].c_str()), scales = atoi(explode("\n", t_[16])[0].c_str());
							if (scales)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_MATERIAL_SCALES;
							else if (feathers)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_MATERIAL_FEATHERS;
							else if (blades)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_1_MATERIAL_BLADES;
							...
							if (open_wings2) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_OPEN_WINGS;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_OPEN_WINGS;

							bool closed_wings2 = atoi(explode("\n", t_[20])[0].c_str());
							if (closed_wings2) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_CLOSE_WINGS;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_CLOSE_WINGS;

							bool stamp_particle2 = atoi(explode("\n", t_[21])[0].c_str());
							if (stamp_particle2) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_STAMP_PARTICLE;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_STAMP_PARTICLE;

							bool trail02 = atoi(explode("\n", t_[22])[0].c_str());
							if (trail02) pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_TRAIL_ON;
							else pInfo(peer)->_flags &= ~GTPH::RIFTWINGS_FLAGS_STYLE_2_TRAIL_ON;


							bool p_aura2 = atoi(explode("\n", t_[23])[0].c_str()), starfield_Aura2 = atoi(explode("\n", t_[24])[0].c_str()), electrical_aura2 = atoi(explode("\n", t_[25])[0].c_str());
							if (electrical_aura2)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_ELECTRICAL_AURA;
							else if (p_aura2)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_PORTAL_AURA;
							...
							if (scales2)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_MATERIAL_SCALES;
							else if (feathers2)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_MATERIAL_FEATHERS;
							else if (blades2)
								pInfo(peer)->_flags |= GTPH::RIFTWINGS_FLAGS_STYLE_2_MATERIAL_BLADES;
						...
						if (t_.size() == 23) {
							...
							bool time_dilation = atoi(explode("\n", t_[4])[0].c_str());
							if (time_dilation) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_TIME_DILATION_ON;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_TIME_DILATION_ON;
							...
							if (cape_collar) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_1_COLLAR_ON;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_1_COLLAR_ON;
							...
							if (closed_cape) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_1_CLOSED_CAPE;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_1_CLOSED_CAPE;


							bool open_cape = atoi(explode("\n", t_[9])[0].c_str());
							if (open_cape) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_1_OPEN_CAPE_ON_MOVEMENT;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_1_OPEN_CAPE_ON_MOVEMENT;

							bool aura_on = atoi(explode("\n", t_[10])[0].c_str());
							if (aura_on) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_1_AURA_ON;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_1_AURA_ON;

							bool p_aura = atoi(explode("\n", t_[11])[0].c_str()), starfield_Aura = atoi(explode("\n", t_[12])[0].c_str()), electrical_aura = atoi(explode("\n", t_[13])[0].c_str());

							pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_1_PORTAL_AURA;
							pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_2_PORTAL_AURA;
							pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_1_STARFIELD_AURA;
							pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_2_STARFIELD_AURA;

							if (electrical_aura)
								pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_1_ELECTRICAL_AURA;
							else if (p_aura)
								pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_1_PORTAL_AURA;
							else if (starfield_Aura)
								pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_1_STARFIELD_AURA;
							...
							if (collar_aura) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_2_COLLAR_ON;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_2_COLLAR_ON;
							...
							if (closed_cape2) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_2_CLOSED_CAPE;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_2_CLOSED_CAPE;

							bool cape_movement2 = atoi(explode("\n", t_[18])[0].c_str());
							if (cape_movement2) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_2_OPEN_CAPE_ON_MOVEMENT;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_2_OPEN_CAPE_ON_MOVEMENT;

							bool aura_on_2 = atoi(explode("\n", t_[19])[0].c_str());
							if (aura_on_2) pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_2_AURA_ON;
							else pInfo(peer)->flags &= ~GTPH::RIFTCAPE_FLAGS_STYLE_2_AURA_ON;

							bool p_aura_2 = atoi(explode("\n", t_[20])[0].c_str()), starfield_aura_2 = atoi(explode("\n", t_[21])[0].c_str()), electrical_2 = atoi(explode("\n", t_[22])[0].c_str());


							if (electrical_2)
								pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_2_ELECTRICAL_AURA;
							else if (p_aura_2)
								pInfo(peer)->flags |= GTPH::RIFTCAPE_FLAGS_STYLE_2_PORTAL_AURA;
						...
						if (pInfo(peer)->cheater_ or has_playmod2(pInfo(peer), 143) or has_playmod2(pInfo(peer), 153)) {
							...
							if (atoi(explode("\n", t_[4 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_16;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_16;

							if (atoi(explode("\n", t_[5 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_3;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_3;


							if (atoi(explode("\n", t_[6 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_4;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_4;
							if (atoi(explode("\n", t_[7 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_2;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_2;
							if (atoi(explode("\n", t_[8+ (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_7;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_7;
							if (atoi(explode("\n", t_[9 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_8;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_8;
							if (atoi(explode("\n", t_[10 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_9;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_9;
							if (atoi(explode("\n", t_[11 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_10;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_10;
							if (atoi(explode("\n", t_[12 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_11;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_11;
							if (atoi(explode("\n", t_[13 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_6;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_6;
							/*
							if (atoi(explode("\n", t_[13 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_12;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_12;
							if (atoi(explode("\n", t_[14 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_13;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_13;*/
							if (atoi(explode("\n", t_[14 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_14;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_14;
							if (atoi(explode("\n", t_[15 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_5;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_5;
							if (atoi(explode("\n", t_[16 + (choose_autofarm ? 1 : 0)])[0].c_str())) pInfo(peer)->cheater_settings |= GTPH::SETTINGS_15;
							else pInfo(peer)->cheater_settings &= ~GTPH::SETTINGS_15;
					...
					case 0: /*Kai zaidejas pajuda*/
					{
						...
						if (p_->characterState == 268435472 || float(p_->characterState) == 80 || float(p_->characterState) == 6.71091e+07 || float(p_->characterState) == 6.71113e+07 || p_->characterState == 268435488 || p_->characterState == 262208 || p_->characterState == 327680 || p_->characterState == 673712008 || p_->characterState == 67371216 || p_->characterState == 268435504 || p_->characterState == 268435616 || p_->characterState == 268435632 || p_->characterState == 268435456 || p_->characterState == 224 || p_->characterState == 112 || p_->characterState == 80 || p_->characterState == 96 || p_->characterState == 224 || p_->characterState == 65584 || p_->characterState == 65712 || p_->characterState == 65696 || p_->characterState == 65536 || p_->characterState == 65552 || p_->characterState == 65568 || p_->characterState == 65680 || p_->characterState == 192 || p_->characterState == 65664 || p_->characterState == 65600 || p_->characterState == 67860 || p_->characterState == 64) {
							if (pInfo(peer)->lava_time + 5000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count()) {
								...
								if (pInfo(peer)->xenonite & GTPH::XENONITE_FORCE_HEAT_RESIST || pInfo(peer)->xenonite & GTPH::XENONITE_BLOCK_HEAT_RESIST || (pInfo(peer)->cheater_settings & GTPH::SETTINGS_8 && pInfo(peer)->disable_cheater == 0)) {
									if (pInfo(peer)->lavaeffect >= (pInfo(peer)->xenonite & GTPH::XENONITE_FORCE_HEAT_RESIST or (pInfo(peer)->cheater_settings & GTPH::SETTINGS_8 && pInfo(peer)->disable_cheater == 0) ? 7 : 3)) {
										pInfo(peer)->lavaeffect = 0;
							...
							for (int i_ = 0; i_ < world_->drop_new.size(); i_++) {
								if (world_->drop_new[i_][2] == p_->plantingTree) {
									...
									if (noclipas == false) {
										...
										if (world_->world_settings & GTPH::SETTINGS_2 && block_access(peer, world_, block_) == false) continue;
