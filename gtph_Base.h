...
bool Carnival = true; // World Event
bool Growganoth = true; // World Event
bool pineapple_event = false; // World Event
bool summer_event = false; // World Event
bool valentine_event = false; // World Event
bool harvest_event = false; // World Event
bool farm_event = true; // Clash Event
// others event
bool blarney_event = false; // World Event
bool beachparty_event = false; // World Event
bool lunar_event = false; // World Event
bool winterfest_event = false; // World Event
...
void items_dat2() {
	string n_ = "database/items/items2.dat";
...
int items_dat() {
	items_dat2();
	string n_ = "database/items/items.dat";
	ifstream file(n_, ios::binary | ios::ate);
	...
	//	if (def.actionType == 98) cout << def.id << endl;
		...
		if (server_port == 16154) items[i].gtwl *= 1000;
		if (items[i].gtwl != 0) {
			items[i].gtwl *= 1.1;
	...
	/*
	{
		ifstream file_info("database/item_prices.txt");
	...
	{
		ofstream m;
		m.open("database/swx.txt"), m << available_items, m.close();
	}*/
	...
	if (file_info.is_open()) {
		...
		file_info.close();
		{
			ifstream file_info("database/items/en");
			if (file_info.is_open()) {
				string line_info;
				while (getline(file_info, line_info)) {
			...
			file_info.close();
			{
				ifstream file_info("database/items/farmables.txt");
				if (file_info.is_open()) {
					string line_info;
					while (getline(file_info, line_info)) {
						for (int i_ = 0; i_ < items.size(); i_++) {
				...
				file_info.close();
				{
					ifstream file_info("database/items/combiner.txt");
					if (file_info.is_open()) {
						string line_info, a = "";
						while (getline(file_info, line_info)) {
							vector<vector<int>> form_recipe;
