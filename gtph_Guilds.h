...
void save_guilds() {
	...
	for (Guild to_save : guilds) {
		if (to_save.guild_settings & GTPH::SETTINGS_0) {
			to_save.guild_settings &= ~GTPH::SETTINGS_0;
			json save_;
...
bool guild_access(const ENetPeer* peer, const uint32_t guild_id) {
	...
	if (p != guilds.end()) {
		...
		if (guild_information->guild_settings & GTPH::SETTINGS_1) {
			return true;
		}
		else {
			if (guild_information->guild_settings & GTPH::SETTINGS_2 and my_rank >= 1 and my_rank != 3 or guild_information->guild_settings & GTPH::SETTINGS_3 and my_rank == 2) {
				return true;
			}
		}
	}
